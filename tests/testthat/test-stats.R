...
test_that("compute_mohns_rho: basic case matches hand calculation", {
  # terminal (retro_year = 2015) + two peels (2013, 2014)
  df <- data.frame(
    year = c(2013, 2014, 2015,
             2013, 2014, 2015,
             2013, 2014, 2015),
    retro_year = c(2013, 2013, 2013,  # peel 2013
...
test_that("compute_mohns_rho: stepped-forward convention (+1 year shift) handled", {
  # Here retro_year labels are one year behind the estimate year used.
  # Function will add +1 to retro_year if max(year) > max(retro_year).
  df <- data.frame(
    year = c(2013, 2014, 2015,
             2013, 2014, 2015,
             2013, 2014, 2015),
    retro_year = c(2012, 2012, 2012,  # peel labeled as 2012 -> shifted to 2013
...
test_that("compute_mohns_rho: zero terminal estimates produce Inf/NaN (documented behavior)", {
  df2 <- data.frame(
    ...
    retro_year = c(2014, 2015, 2016),  # terminal is retro_year == 2016, est_t at 2015 = 0
...
test_that("compute_mohns_rho: retro_year coercion works (factor/character)", {
  df_char <- data.frame(
    year = c(2013, 2014, 2013, 2014),
    retro_year = c("2013", "2013", "2014", "2014"),
    est = c(90, NA, 100, 100)  # terminal retro_year = "2014"
  )
  expect_silent(rho_char <- compute_mohns_rho(df_char))
  expect_true(is.finite(rho_char))

  df_fac <- df_char
  df_fac$retro_year <- factor(df_fac$retro_year)
